...
async function migrate() {
  try {
    ...
    const existing = await SectorUnit.findOne({ where: { name: 'Transport and Logistics' } });
    if (existing) {
      console.log('Transport and Logistics already exists (id=' + existing.id + '). Nothing to do.');
      process.exit(0);
    }
    ...
    const unit = await SectorUnit.create({ name: 'Transport and Logistics', sectorTypeId: instType.id, parentId: null });
    for (const catId of catIds) {
      await SectorUnitCategory.create({ sectorUnitId: unit.id, memberCategoryId: catId });
    }

    console.log('✅ Transport and Logistics sector unit created (id=' + unit.id + ')');
    process.exit(0);
  } catch (err) {
