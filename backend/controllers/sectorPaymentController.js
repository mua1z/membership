...
exports.deletePayment = async (req, res) => {
  try {
    ...
    // Optional: Delete receipt file
    if (payment.receiptFile) {
      const oldPath = path.join(__dirname, '..', 'uploads', 'receipts', payment.receiptFile);
      if (fs.existsSync(oldPath)) {
        try { fs.unlinkSync(oldPath); } catch (e) { console.error('Failed to delete receipt file:', e); }
      }
    }

    // Must delete child records (audit logs) first before the parent record due to foreign key constraints
    await SectorPaymentAuditLog.destroy({ where: { sectorPaymentId: req.params.id } });

    await payment.destroy();

    res.json({ success: true, message: 'Sector deposit deleted successfully.' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
